...
| ~~`ADMIN_DATABASE_URL` ausente no `.env` da VM~~ | `/opt/barbeariapro/.env.docker` | ✅ Resolvido | D-93 (2026-08-04): provisionada em `.env.docker` (mesmo arquivo do `DATABASE_URL` → `host.docker.internal`, role `postgres`, senha de `POSTGRES_PASSWORD`), backup do arquivo anterior feito antes. Validado via `docker compose -f docker-compose.app.yml run --rm --no-deps backend` conectando como `postgres`. `deploy/update.sh` segue não testado end-to-end (ver se ainda quebra no passo de migration). |
