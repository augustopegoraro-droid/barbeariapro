...
| Select dentro de Dialog pode fechar o Dialog inteiro ao escolher uma opção | `components/ui/select.tsx` + `components/ui/dialog.tsx` | ⚠️ Médio | Achado durante o teste em browser da Fase 2 de Produtos/Estoque (2026-08-03): reproduzido tanto no `MovimentacaoDialog` novo quanto no `ProdutoFormDialog` já em produção (categoria) — pré-existente, não é regressão desta sessão. Não investigado a fundo (mudança em primitivo compartilhado exige plano próprio); pode ser artefato da ferramenta de automação de clique — confirmar com clique real antes de priorizar o conserto. |
